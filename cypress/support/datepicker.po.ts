...
export class DatepickerPo extends BaseComponent {
  ...
  /**
   * This method check navigation bar for date- and range- picker, including nav-arrows, nav-month, nav-year
   * Each element checked by params of visibility/hidden, text, value
   */
  ...
  isDatepickerOpened(baseSelector: string, opened: boolean) {
  ...
  clickOnDaterangePickerTableItem(mode: string, itemIndex?: number, itemText?: string) {
    const bodyView = this.getBodyParams(mode).bodyView;
  ...
  } //TODO need add correct functionality
}
